...
class SFTPHandler(asyncssh.SFTPServer):
    ...
    async def open(self, path, pflags, attrs):
        ...
        # Determine file open mode from SFTP flags
        if pflags & asyncssh.FXF_WRITE:
            if pflags & asyncssh.FXF_APPEND:
                mode = 'ab'
            elif pflags & asyncssh.FXF_CREAT:
                mode = 'wb' if (pflags & asyncssh.FXF_TRUNC) else 'xb'
            else:
                mode = 'r+b'
        else:
            mode = 'rb'
        ...
        try:
            ...
            raise asyncssh.SFTPNoSuchFile(f"File not found: {path}")
